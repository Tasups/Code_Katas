/*
Write a function that takes an array of numbers and finds all prime numbers from that array and returns them in a sorted array. 
...
2. I could write my own sorting algorithm, however, I think that .sort((a,b) => a - b) should work well.
*/
...
// /**
//  * THIS IS A FUNCTION TO FIND THE PRIMES FROM 2 TO N WHERE THE INPUT IS A SINGLE NUMBER INSTEAD OF AN ARRAY
//  * 
//  */

// function numFindPrimes(num) {
//     let result = [];
//     let count = 2;
//     while (count < Math.floor(num / 2)) {
//         if()
//         count++;
//     }
//   console.log(util.inspect(result, { maxArrayLength: null }));
 ...
 * 
 * We need to go through each number in the array, array[i] and find out how many lesser prime numbers can go into the number itself. 
 * Looping will be necessary, but once again, we don't have to go to the actual number, we can split the number in half, round down and start the looping there.
 * We could use a while loop and make the condition be while (count < Math.floor(array[i]) / 2);
 * We could use a counter or divisor that goes up to the next highest prime number. The only problem with this is to find the next highest prime number. We could write an algorithm using a Sieve of Eratosthenes to find many prime numbers, however, we have already made a prime number finder, so we could tweak it a little and use it to find all the prime numbers from 2 to half the target number. 
 * We need to create a function that creates an array of numbers, sorted, from 2 to n. That is why I created the helper function printToANumber, so that we can make an array and then pass it to our previous function sortedPrimes. The sorting may be redundant for this application, but that's ok, we can still use it. I did use printToANumber for testing of the sortedArray function as well.
 * 
 */
